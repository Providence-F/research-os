...
def render_dashboard(model: dict[str, Any]) -> str:
    visual_modules = model.get("visual_modules") or []
    # 如果声明了 visual_modules，只渲染声明的模块；否则回退到渲染所有非空模块
    has_filter = bool(visual_modules)

    def should_render(name):
        if not has_filter:
            return True
        return name in visual_modules
    ...
    if should_render("hero"):
        parts.append(render_hero(model))
    if should_render("summary_cards"):
        parts.append(render_summary_cards(model.get("summary_cards") or []))
    if should_render("object_cards") and cards:
        parts.append(render_object_cards(cards, model.get("object_kind") or "object"))
    if should_render("strategy_tabs"):
        parts.append(render_tabs(model.get("strategy_tabs") or model.get("tabs") or []))
    if should_render("comparison_matrix") and matrix:
        parts.append(render_matrix(matrix))
    if should_render("filterable_table") and table:
        parts.append(render_filterable_table(table))
...
<body data-source="final-report.md" data-view-type="{html.escape(str(state.get('view_type', '')))}" data-theme="{html.escape(str(layout_spec.get('theme', '')))}" data-layout-v07="{'true' if has_layout_spec else 'false'}">
  ...
  <div class="page-shell">
    ...
    <main>
      ...
      <div class="subtitle">{((view_model or {}).get('hero') or {}).get('summary') or ''}</div>
      {dashboard}
      {full_report_open}
